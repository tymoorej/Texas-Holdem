...
def player_bet(current_call,player,bot,table,can_raise=True,done=False):
    ...
    while end is False:
        ...
        player_chips = myfont.render("Player chips: " +str(player.get_chips()), 1, white)
        Window.blit(player_chips, (800, 460))

        bot_chips = myfont.render("Bot chips: " +str(bot.get_chips()), 1, white)
        Window.blit(bot_chips, (800, 200))

        table_chips = myfont.render("Table chips: " +str(table.get_chips()), 1, white)
...
def bet_call(player,bot,table):
    ...
    if type(d) is tuple:
        ...
        if type(d[1]) is Player:
            table.give_pot(bot)
        skip_to_end=2
    print('p: ',player.get_chips(),'b: ',bot.get_chips(),'t: ',table.get_chips())
    print('\n'*3)
    return skip_to_end
...
def bet(player,bot,table):
    ...
    while True:
        print('Player turn')
        if bot.get_chips()==0:
            current_call=player_bet(current_call,player,bot,table,can_raise=False)
            if game_over:
                pygame.quit()
                quit()
        else:
