...
def printable_dict_of_winners(table,players):
    ...
    value_to_meaning={10:'Royal Flush',9: 'Straight Flush',\
...
def winner(table,players,printing=True):
    ...
    sorted_players=sorted(values.items(),key=lambda k:k[1], reverse=True) # O(p*log(p))
...
def has_flush(table,player):
    ...
    for c in player.get_cards(): # O(n)
        if c[1]=='Hearts':
            ...
            Clubs.add(c)
    if len(Hearts) >=5 or len(Diamonds) >=5 or len(Spades) >=5 or len(Clubs) >=5:
        return True
    return False
